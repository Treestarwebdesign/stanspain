 ...
 * Description: <strong>Heads up!</strong> As of v2.2.3 MultilingualPress PRO has been functionally harmonized with MultilingualPress (free). <a href="https://marketpress.com/2015/multilingual-wordpress-free/"><strong>Learn more about the v2.2.3 update here</strong></a>
 * Author:      Inpsyde GmbH
...
function mlp_init() {
	...
	// check for merge
	$parent_path = dirname( $plugin_path ) . '/';
	$target_folder = 'multilingual-press';
	$merge_status = get_site_option( 'mlp_merge_attempted', FALSE );
	// only if free version is not available
	if ( ! file_exists( $parent_path . $target_folder ) && $merge_status != '1' ) {
		// try and be quiet if anything will fail, we will work around then
		$rename_status = @rename( $plugin_path . $target_folder, $parent_path . $target_folder );
		// we only try that one time
		update_site_option( 'mlp_merge_attempted', '1' );
		if ( $rename_status == TRUE ) {
			// activate plugin
			$current = get_site_option('active_sitewide_plugins', array());
			$current[ $target_folder . '/' . $target_folder . '.php' ] = time();
			//deactivate pro
			unset( $current[ $target_folder . '-pro/' . $target_folder . '.php' ] );
			// save sitewide plugins
			update_site_option( 'active_sitewide_plugins', $current );
			update_site_option( 'mlp_merge_done', '1' );
		}
	}
